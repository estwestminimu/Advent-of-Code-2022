...
for (i = 0; i < array.length; i++) {
  //dlugosc stringu
  for (z = 0; z < array[i].length / 2; z++) {
    if (z == 0) {
      array2[p] = "";
    }

    array2[p] += array[i].charAt(z);
  }
  p++;
  for (z = array[i].length / 2; z < array[i].length; z++) {
    if (z == array[i].length / 2) {
      array2[p] = "";
    }
...
var flag = false;
//w array2
for (u = 0; u < array2.length; u += 2) {
  //w stringu
  for (v = 0; v < array2[u].length; v++) {
    for (let z = 0; z < array2[u + 1].length; z++) {
      if (array2[u].charAt(v) == array2[u + 1].charAt(z)) {
        out[f] = array2[u].charAt(v);
        f++;
        flag = true;
        break;
      }
